...
QString SHA1::getDigest()
{
	...
	QString qShaDigest = (const char*)digest;

	QByteArray hexDigestData = qShaDigest.toAscii();

    QString hexData = "";

    foreach (uint byte, hexDigestData)
    {
    	if (hexData.length() < 128)
        {
    	    QString hex = QString("%1").arg(byte, 2, 16, QLatin1Char('0')).toUpper();
            hexData.append(hex.right(2) + " ");
        }
    }


//	QByteArray hexData = hexdata.toAscii();

	// return the digest
//	return (unsigned char*)hexData;
